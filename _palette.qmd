...
// Helper function to display a color swatch with info
#let color-swatch(hex, size: 40pt, show-name: true, show-cat: false) = {
  ...
  if show-cat {
    content = [#content -- #info.cat]
  }
  
  stack(
    dir: ltr,
    spacing: 6pt,
    square(size: size, fill: rgb(hex)),
    if show-name or show-cat { content }
  )
}
...
#for (name, colors) in categorical-palettes {
  ...
  grid(
    ...
    ..colors.map(hex => {
      let info = get-color-info(hex)
      stack(
        square(size: 40pt, fill: rgb(hex)),
        text(size: 7pt, [#info.name]),
        text(size: 6pt, fill: gray, [#hex])
      )
    })
  )
}
